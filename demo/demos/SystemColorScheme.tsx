...
import { Chart, Series, Scale, Axis, Legend, Tooltip } from 'uplot-plus';
  ...
  return (
    <div>
      ...
      <div className="uplot-auto-scheme">
        <Chart width={780} height={340} data={data} title="Auto Light/Dark">
          <Scale id="x" />
          <Scale id="y" />
          <Axis scale="x" />
          <Axis scale="y" />
          <Series group={0} index={0} label="Revenue" />
          <Series group={0} index={1} label="Growth" />
          <Legend />
          <Tooltip />
        </Chart>
      </div>
      ...
      <div className="mt-4">
        ...
        <pre className="text-xs bg-gray-100 p-3 rounded overflow-auto max-h-52">{cssText.trim()}</pre>
      </div>
    </div>
  );
}
