...
describe('getIncrSpace', () => {
  it('returns [0, 0] for zero dimension', () => {
    const cfg: AxisConfig = { scaleId: 'y', side: Side.Left };
    expect(getIncrSpace(cfg, 0, 100, 0)).toEqual([0, 0]);
  });

  it('vertical axis uses 30px spacing', () => {
    const cfg: AxisConfig = { scaleId: 'y', side: Side.Right };
  ...
  it('horizontal axis estimates space from labels', () => {
    const cfg: AxisConfig = { scaleId: 'x', side: Side.Bottom };
  ...
  it('respects custom space', () => {
    const cfg: AxisConfig = { scaleId: 'y', side: Side.Right, space: 60 };
...
describe('computeAxisSize', () => {
  it('returns fixed size when specified', () => {
    const cfg: AxisConfig = { scaleId: 'y', side: Side.Right, size: 80 };
  ...
  it('accounts for hidden ticks', () => {
    const cfg: AxisConfig = { scaleId: 'y', side: Side.Right, ticks: { show: false } };
    const withTicks: AxisConfig = { scaleId: 'y', side: Side.Right, ticks: { show: true, size: 10 } };
...
describe('createAxisState', () => {
  ...
  it('hidden axis', () => {
    const state = createAxisState({ scaleId: 'y', side: Side.Right, show: false });
